...
export const AddExpenseDialog = ({
  ...
  const addExpensesMutation = useMutation({
    mutationFn: async () => {
      await Promise.all(
        expenses.map(async (e) => {
          ...
          const recordPayload = {
            id: e.id, // e.id is a guaranteed UUID generated by the UI state
            ...
            user_id: userId,
            bill_url: billUrl,
            // business fields handled safely here implicitly or explicitly through db schemas
    ...
    onSuccess: () => {
      // Optimistically inject the newly added items into the UI cache so they appear immediately while offline
      if (userId) {
