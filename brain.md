...
9. **Notes Column & Payment Transcript**: `public.sales` and `public.purchases` both have `notes TEXT` column used for invoice terms and structured payment audit entries (`<!-- FINFLOW_PAYMENTS:[...] -->`). `parsePaymentTranscript()` and `encodePaymentTranscript()` (`paymentTranscript.ts`) serialize payment voucher ledgers safely while preserving user notes, maintaining 100% resilience across offline SQLite and Supabase sync.
...
13. **CA Bill Payment & Universal Transaction Ledger**:
    - **Payment In (Sales Receipts)**: Discharges debtor receivables (Dr. Cash/Bank, Cr. Customer). Available via header "+ Payment In" button and row-level "Payment In" actions on invoices.
    - **Payment Out (Purchase Vouchers)**: Discharges creditor payables (Dr. Vendor, Cr. Cash/Bank). Available via header "+ Payment Out" button and row-level "Payment Out" actions on bills.
    - **With Bill (Synchronized)**: Selecting a pending/partial bill recalculates `amount_paid`, `balance_due`, `status` (`paid` vs `partial`), and embeds sequential voucher (`REC-YYYYMMDD-XXXX` or `PAY-YYYYMMDD-XXXX`) into the bill's `notes` payment transcript.
    - **Without Bill (On Account / Advance)**: Creates an immutable standalone transaction record (`sales` document_type: 'receipt' or `purchases` payment voucher) and adjusts `party.opening_balance` so customer receivable / vendor payable drops immediately. Every transaction without exception is recorded in database and audit ledger.
    - **Audit Ledgers**: Handled via `UniversalPaymentDialog.tsx`, `RecordBillPaymentDialog.tsx` (adapter), and `BillPaymentTranscriptDialog.tsx` (chronological audit trail with settlement % progress bar and print/copy).