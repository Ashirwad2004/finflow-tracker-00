...
CREATE POLICY "Group members can add new members" ON public.group_members
...
CREATE POLICY "Group members can view all group expenses" ON public.group_expenses
FOR SELECT USING (
  auth.uid() IN (
...
CREATE POLICY "Group members can add expenses" ON public.group_expenses
FOR INSERT WITH CHECK (
  auth.uid() IN (
    SELECT user_id FROM public.group_members WHERE group_id = group_expenses.group_id
...
CREATE POLICY "Users can delete their own group expenses" ON public.group_expenses
FOR DELETE USING (
  auth.uid() IN (
    SELECT user_id FROM public.group_members WHERE group_id = group_expenses.group_id
  ) AND auth.uid() = user_id
...
ADD CONSTRAINT IF NOT EXISTS fk_group_expenses_group_id
FOREIGN KEY (group_id) REFERENCES public.groups(id) ON DELETE CASCADE;

ALTER TABLE public.group_expenses
ADD CONSTRAINT IF NOT EXISTS fk_group_expenses_user_id
FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE public.group_expenses
ADD CONSTRAINT IF NOT EXISTS fk_group_expenses_category_id
