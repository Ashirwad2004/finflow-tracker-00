...
-- Enable RLS on lent_money
ALTER TABLE public.lent_money ENABLE ROW LEVEL SECURITY;

-- RLS policies for lent_money
CREATE POLICY "Users can view their own lent money records" 
ON public.lent_money 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own lent money records" 
ON public.lent_money 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own lent money records" 
ON public.lent_money 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own lent money records" 
ON public.lent_money 
FOR DELETE 
USING (auth.uid() = user_id);

-- Create trigger for updated_at on lent_money
